...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const sidebarMenu = document.querySelector('.sidebar-main-menu');

    if (sidebarMenu) {
        const savedScrollTop = sessionStorage.getItem('uptmsSidebarScrollTop');

        if (savedScrollTop !== null) {
            requestAnimationFrame(function () {
                sidebarMenu.scrollTop = Number(savedScrollTop);
            });
        } else {
            const activeItem = sidebarMenu.querySelector(
                '.sidebar-menu-link.active, .sidebar-submenu li.active'
            );

            ...
            <div class="row g-3">
                <div class="col-12 col-md-6">
                    <div class="table-search-box"><i class="bi bi-search table-search-icon"></i><input type="text"
                            class="table-search-input" placeholder="Search thesis or author..."></div>
                </div>
            ...
            <table class="table-custom">
                ...
                <tbody>
                    <?php foreach ($reports as $report): ?><tr>
                        <td>
                            <div class="table-user-name"><?= htmlspecialchars($report['title']) ?></div>
                        </td>
                        <td><?= htmlspecialchars($report['author']) ?></td>
                        <td><?= htmlspecialchars($report['department']) ?></td>
                        <td><?= htmlspecialchars($report['year']) ?></td>
                        <td><?= htmlspecialchars($report['supervisor']) ?></td>
                        <td><?php if ($report['status'] === 'Published'): ?><span
                                class="badge-table success">Published</span><?php else: ?><span
                                class="badge-table pending">Under Review</span><?php endif; ?></td>
                        <td class="text-center"><a href="#" class="table-btn-action" title="View thesis report"
                                aria-label="View thesis report"><i class="bi bi-eye"></i></a></td>
                    </tr><?php endforeach; ?>
                </tbody>
            </table>
        </div>
        <div class="table-footer-control">
            <div class="table-pagination-info">Showing 1 to <?= count($reports) ?> of <?= count($reports) ?> thesis
                reports</div>
            <nav aria-label="Thesis report pagination">

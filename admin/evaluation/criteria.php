    ...
    <div class="table-card-custom">
            ...
            <div class="row g-3">
                <div class="col-12 col-md-7">
                    <div class="table-search-box"><i class="bi bi-search table-search-icon"></i><input type="text"
                            class="table-search-input" placeholder="Search evaluation criteria..."></div>
                </div>
                <div class="col-12 col-md-3">
                    <select class="form-select-custom">
                        <option value="">All Categories</option>
                        <option>Project Evaluation</option>
                        <option>Thesis Evaluation</option>
                    </select>
                </div>
        ...
        <div class="table-responsive">
            <table class="table-custom">
                ...
                <tbody>
                    <?php foreach ($criteria as $item): ?><tr>
                        <td>
                            <div class="table-user-name"><?= htmlspecialchars($item['name']) ?></div>
                        </td>
                        <td><?= htmlspecialchars($item['category']) ?></td>
                        <td><strong><?= htmlspecialchars($item['weight']) ?></strong></td>
                        <td><?= htmlspecialchars($item['description']) ?></td>
                        <td><?php if ($item['status'] === 'active'): ?><span
                                class="badge-table success">Active</span><?php else: ?><span
                                class="badge-table pending">Inactive</span><?php endif; ?></td>
                        <td>
                            <div class="d-flex justify-content-center gap-1"><a href="#" class="table-btn-action"
                                    title="Edit criteria" aria-label="Edit criteria"><i
                                        class="bi bi-pencil"></i></a><button type="button"
                                    class="table-btn-action delete" title="Delete criteria"
                                    aria-label="Delete criteria"><i class="bi bi-trash"></i></button></div>
                        </td>
                    </tr><?php endforeach; ?>
                </tbody>
            </table>
        ...
        <div class="table-footer-control">
            <div class="table-pagination-info">Showing 1 to <?= count($criteria) ?> of <?= count($criteria) ?> criteria
            </div>
            <nav aria-label="Criteria pagination">

    ...
    <div class="table-card-custom">
            ...
            <div class="row g-3">
                <div class="col-12 col-md-7">
                    <div class="table-search-box"><i class="bi bi-search table-search-icon"></i><input type="text"
                            class="table-search-input" placeholder="Search milestone or project..."></div>
                </div>
        ...
        <div class="table-responsive">
            <table class="table-custom">
                ...
                <tbody>
                    <?php foreach ($milestones as $milestone): ?><tr>
                        <td>
                            <div class="table-user-name"><?= htmlspecialchars($milestone['name']) ?></div>
                        </td>
                        <td><?= htmlspecialchars($milestone['project']) ?></td>
                        <td><?= htmlspecialchars($milestone['due']) ?></td>
                        <td style="min-width: 150px;">
                            <div class="d-flex justify-content-between small mb-1">
                                <span><?= $milestone['completion'] ?>%</span></div>
                            <div class="progress" style="height: 6px;">
                                <div class="progress-bar bg-primary" role="progressbar"
                                    style="width: <?= $milestone['completion'] ?>%;"
                                    aria-valuenow="<?= $milestone['completion'] ?>" aria-valuemin="0"
                                    aria-valuemax="100"></div>
                            </div>
                        </td>
                        <td><?php if ($milestone['status'] === 'completed'): ?><span
                                class="badge bg-success-subtle text-success">Completed</span><?php else: ?><span
                                class="badge bg-primary-subtle text-primary">Ongoing</span><?php endif; ?></td>
                        <td class="text-end"><a href="#" class="table-btn-action" title="Edit milestone"
                                aria-label="Edit milestone"><i class="bi bi-pencil"></i></a><button type="button"
                                class="table-btn-action delete" title="Delete milestone"
                                aria-label="Delete milestone"><i class="bi bi-trash"></i></button></td>
                    </tr><?php endforeach; ?>
                </tbody>
            </table>
        </div>
        <div class="table-footer-control">
            <div class="table-pagination-info">Showing 1 to <?= count($milestones) ?> of <?= count($milestones) ?>
                milestones</div>
